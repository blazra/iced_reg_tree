...
use iced::{
    Element, Length, Padding, Rectangle, Size, Theme, Vector,
};
    ...
    T: Display + Clone,
{
    ...
    /// Creates a new [`State`] for a [`ComboBox`] with the given list of options
    /// and initial text.
    pub fn with_text(options: Vec<T>, value: String) -> Self {
        // Pre-build "matcher" strings ahead of time so that search is fast
        let option_matchers = build_matchers(&options);
    ...
    Renderer: text::Renderer,
{
    fn size(&self) -> Size<Length> {
        Size { width: Length::Fixed(0.0), height: Length::Fixed(0.0) }
    }
